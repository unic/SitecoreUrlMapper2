...
function Install-Prerequisites {
    ...
    try
    {
        Invoke-Sqlcmd -ServerInstance $SqlServer `
                      ...
                      -InputFile "$DatabaseRoot\containedauthentication.sql"
    }
    catch
    {
    ...
    if(-not (Test-Path "$SolrRoot")) {
        throw "The Solr root path '$SolrRoot' appears invalid."
    }
...
function Install-XP0SingleDeveloper {
    ...
    $singleDeveloperParams = @{
        ...
        JSSServerPackage = $JSSServerPackage
        PSEPackage = $PSEPackage
    }
    try {
